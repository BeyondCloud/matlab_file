...
colorbar;
caxis([0 0.05]);
xlabel(['Distance(' num2str(dx) 'm)']) ; ylabel(['Distance(' num2str(dx) 'm)'])
shading interp;
           ...
           %const1 = ( r *( c ^2) /dx ) ; 
            ...
            % const2 = (1/( dx* r));
             vx2 (i,j) = dt /(1+ox(i,1)*dt)*...
                 ((vx0 (i,j)*( 1/ dt))+(const2 *(p(i,j)-p(i-1, j)))) ;
             vy2 (i,j) = ( dt  /(1+oy(1,j)*dt))* ...
                 ((vy0(i,j)*( 1/dt))+(const2 *(p(i,j)-p(i,j-1))));
             vx2(wall)  = 0;
             vy2(wall)  = 0;
